<x-app-layout>
    <x-slot name="searchBar">
        <x-search-input placeholder="Search by Question" value="{{ isset($search) ? $search : '' }}" />
    </x-slot>
    <x-slot name="header">
        <!-- Breadcrumbs -->
        ...
        <div>
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-lg">
                <div class="text-gray-900 dark:text-gray-100">
                    ...
                    @endif
                    <div id="faq-list">
                        @include('components.faq.faq-list', ['faqs' => $data])
                    </div>
                </div>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        searchInput.addEventListener('keyup', function() {
            ...
            fetch(`{{ route('admin.faq-management') }}?search=${query}`, {
                    headers: {
                ...
                .then(data => {
                    document.getElementById('faq-list').innerHTML = data;
                });
        });
    });
</script>
