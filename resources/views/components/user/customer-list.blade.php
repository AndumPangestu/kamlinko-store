<div class="flex flex-col gap-4">
    ...
    @if (count($data) == 0)
        <div class="text-gray-600 text-center">
            ...
            <a href="{{ route('admin.user-management.create', ['mode' => 'customer']) }}"
                    ...
                    @foreach ($data as $customer)
                        <tr class="border-t border-gray-200 hover:bg-gray-50">
                            <td class="flex flex-col px-4 py-3 text-sm text-gray-800 truncate">
                                <p class="text-gray-800">{{ \Illuminate\Support\Str::limit($customer->username, 20) }}
                                </p>
                                <p class="text-sm text-gray-400">
                                    {{ \Illuminate\Support\Str::limit($customer->email, 20) }}</p>
                            </td>
                            </td>
                            ...
                            <td class="px-4 py-3 text-sm text-gray-400 truncate">
                                {{ $customer->userAddress ? \Illuminate\Support\Str::limit($customer->userAddress->address, 20) : 'Not Set' }}
                            </td>
                            <td class="px-4 py-3 text-sm text-gray-400 truncate">
                                {{ $customer->userAddress ? \Illuminate\Support\Str::limit($customer->userAddress->city, 20) : 'Not Set' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-400">
                                {{ $customer->userAddress ? $customer->userAddress->zip : 'Not Set' }}
                            </td>
                            <td class="px-4 py-3 text-right">
                                <div class="flex justify-end gap-3 text-gray-600 items-center h-full w-full">
                                    <a href="{{ route('admin.user-management.view-user-details', $customer->id) }}"
                                    ...
                                    <span class="mx-2 cursor-pointer">
                                        <form class="flex items-center" method="POST"
                                            action="{{ route('admin.customer-management.destroy', $customer->id) }}"
