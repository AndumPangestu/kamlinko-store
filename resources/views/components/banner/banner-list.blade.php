<div class="flex flex-col gap-4">
    @if (count($data) == 0)
        <div class="text-gray-600 text-center">
            <p>No banner found.</p>
            <a href="{{ route('admin.banner-management.create') }}" class="text-blue-600 underline">
                Add new Banner
            </a>
        </div>
    @else
            ...
            <table class="min-w-full table-auto">
                ...
                <thead class="divide-y divide-gray-200">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-lg font-semibold text-gray-700 w-1/3">
                            <a class="flex flex-row gap-2 items-center"
                                href="?sort_by=type&sort_order={{ $sortBy === 'type' && $sortOrder === 'asc' ? 'desc' : 'asc' }}">
                                <span>Type</span>
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                    class="{{ $sortBy === 'type' ? 'text-blue-500' : 'text-gray-400' }} {{ $sortBy === 'type' && $sortOrder == 'desc' ? 'scale-x-[-1]' : '' }} bi bi-arrow-down-up"
                                    fill="currentColor" viewBox="0 0 16 16">
                                    <path fill-rule="evenodd"
                                        d="M11.5 15a.5.5 0 0 0 .5-.5V2.707l3.146 3.147a.5.5 0 0 0 .708-.708l-4-4a.5.5 0 0 0-.708 0l-4 4a.5.5 0 1 0 .708.708L11 2.707V14.5a.5.5 0 0 0 .5.5m-7-14a.5.5 0 0 1 .5.5v11.793l3.146-3.147a.5.5 0 0 1 .708.708l-4 4a.5.5 0 0 1-.708 0l-4-4a.5.5 0 0 1 .708-.708L4 13.293V1.5a.5.5 0 0 1 .5-.5" />
                                </svg>
                        ...
                        <th class="px-4 py-3 text-right text-lg font-semibold text-gray-700 w-1/4">
                            <div class="flex flex-row justify-end items-center gap-5">
                                ...
                                <a href="{{ route('admin.banner-management.create') }}"
                    ...
                    @foreach ($data as $banner)
                        <tr class="border-t border-gray-200 hover:bg-gray-50">
                            <td class="px-4 py-3 text-sm text-gray-800 truncate">
                                {{ \Illuminate\Support\Str::limit($banner->name, 50) }}

                            </td>
                            <td class="px-4 py-3 text-sm text-gray-400 truncate">
                                {{ \Illuminate\Support\Str::limit($banner->description, 50) }}
                            </td>
                            <td class="px-4 py-3 text-sm text-gray-400 truncate">
                                {{ \Illuminate\Support\Str::limit($banner->type, 50) }}
                            </td>
                            <td class="px-4 py-3 text-right">
                                <div class="flex justify-end gap-3 text-gray-600 items-center h-full w-full">
                                    <a href="{{ route('admin.banner-management.view-banner-details', $banner->id) }}"
                                    ...
                                    <span class="mx-2 cursor-pointer">
                                        <form class="flex items-center" method="POST"
                                            action="{{ route('admin.banner-management.destroy', $banner->id) }}"
